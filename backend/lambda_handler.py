...
def _handle_ingest_schema(event: Dict[str, Any], origin: Optional[str], schema_id: str) -> Dict[str, Any]:
    ...
    try:
        result = execute_mapping(data, mapping_spec, target_schema)
    except Exception as exc:
        storage.update_job(
            job_id=job_created.id,
            partner_internal_id=int(partner_internal_id),
            status="failed",
            issues=[{"field": "mapping", "message": str(exc)}],
        )
        return _response(400, {"error": str(exc)}, origin=origin)
    ...
    job_updated = storage.update_job(
        job_id=job_created.id,
        partner_internal_id=int(partner_internal_id),
        status="completed",
    ...
    return _response(
        200,
        {"job": _job_to_dict(job_updated or job_created), "result": result},
        origin=origin,
    )
...
def _handle_create_job(event: Dict[str, Any], origin: Optional[str]) -> Dict[str, Any]:
    ...
    return _response(200, {"job": _job_to_dict(job), "result": result}, origin=origin)
